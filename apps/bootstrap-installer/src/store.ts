  ...
  unlisten = await listen<BootstrapEvent>("bootstrap", (event) => {
    ...
    switch (payload.type) {
      ...
      case "log": {
        const logs = [
          ...cur.logs,
          { stage: payload.stage, line: payload.line, stream: payload.stream },
        ];
...
const fakeStage = (
  ...
  $bootstrap.set(
    withStageState($bootstrap.get(), name, state, durationMs, error),
  );

const fakeLog = (stage: string, line: string) =>
  $bootstrap.set({
    ...$bootstrap.get(),
    logs: [...$bootstrap.get().logs, { stage, line, stream: "stdout" }],
  });
...
async function runFakeBoot(kind: FakeMode): Promise<void> {
  ...
  try {
    ...
    $bootstrap.set({
      ...
      stages: Object.fromEntries(
        stages.map((s): [string, StageRecord] => [
          s.name,
          {
            info: { ...s, category: kind, needs_user_input: false },
            state: null,
          },
        ]),
      ),
    ...
    const failAt =
      kind === "failure" ? stages[Math.floor(stages.length / 2)]?.name : null;
    ...
    for (const s of stages) {
      ...
      for (let l = 0; l < lines; l++) {
        ...
        fakeLog(
          s.name,
          `[${s.name}] ${s.title.toLowerCase()} — step ${l + 1}/${lines}…`,
        );
      }
