...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend hermes.exe (or the desktop Atlas.exe
...
/// Force-kill any Atlas or compatibility CLI process other than this process.
...
fn force_kill_other_hermes() {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // /FI excludes our own PID; /T kills the tree; /F forces. Keep the
        // old engine image as a compatibility cleanup for upgraded installs.
        for image in ["Atlas.exe", "hermes.exe"] {
...
/// Path to the branded venv shim, with an upstream-name compatibility fallback.
fn venv_hermes(install_root: &Path) -> PathBuf {
    let atlas = if cfg!(target_os = "windows") {
...
/// Resolve the Atlas CLI to drive. Prefer the venv shim in the install we just
/// updated; fall back to the public command and then the compatibility command.
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    envs.push(("HERMES_PUBLIC_BRAND".to_string(), OsString::from("atlas")));
    envs.push(("HERMES_BIN".to_string(), OsString::from("atlas")));
...
mod tests {
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let tmp = base.join("Atlas.app.hermes-update-new");
        let old = base.join("Atlas.app.hermes-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Atlas.app");
        let tmp = base.join("Atlas.app.hermes-update-new"); // intentionally absent
        let old = base.join("Atlas.app.hermes-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Atlas.app");
        let tmp = base.join("Atlas.app.hermes-update-new"); // absent
        let old = base.join("Atlas.app.hermes-update-old");
        write_marker(&target, "OLD");
