...
//!
//! Mirrors Atlas's branded runtime home:
//!   Windows: %LOCALAPPDATA%\atlas
//!   macOS:   ~/.atlas
//!   Linux:   ~/.atlas  (override via $HERMES_HOME)
//!
//! NOTE (macOS): Python's get_hermes_home(), scripts/install.sh, and the
//! Electron desktop's resolveHermesHome() ALL use ~/.atlas on macOS — there
...
/// Returns the canonical Atlas home directory, respecting $HERMES_HOME if set.
pub fn hermes_home() -> PathBuf {
    ...
    {
        // %LOCALAPPDATA%\atlas — matches scripts/install.ps1's $HermesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("atlas");
        }
    }

    // macOS + Linux + fallback: ~/.atlas (matches the branded Python runtime,
