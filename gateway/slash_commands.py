...
def _finalize_agent_memory_boundary(agent: Any, reason: str) -> None:
    """Durably finalize memory before reset tears the provider down."""
...
def _commit_agent_memory_boundary(
    agent: Any,
    new_session_id: str,
    ...
    """Atomically finalize and prepare a live provider's target lineage."""
    manager = getattr(agent, "_memory_manager", None)
    if manager is None:
        return
    messages = getattr(agent, "_session_messages", None)
...
def _prepare_agent_memory_branch(
    agent: Any,
    new_session_id: str,
    parent_session_id: str,
    reason: str,
    ...
    """Create/rebind a live child lineage without ending its source."""
    del reset
    manager = getattr(agent, "_memory_manager", None)
    if manager is None:
        return
    # Publish compensation intent before the provider mutation. If a provider
    # raises after partially rebinding, the caller can still restore the source
    # and discard the unpublished child deterministically.
    try:
        agent._memory_branch_binding = (parent_session_id, new_session_id)
    except Exception:
        pass
...
def _restore_agent_memory_branch_source(
    agent: Any,
    source_session_id: str,
    child_session_id: str,
) -> None:
    """Undo a live provider rebind after the routing CAS loses."""
    if getattr(agent, "_memory_branch_binding", None) != (
        source_session_id,
        child_session_id,
    ):
        return
    ...
    try:
        del agent._memory_branch_binding
    except Exception:
        pass


def _finalize_detached_cortex_boundary(
    profile_home: Path,
    session_id: str,
    reason: str,
) -> bool:
    """Finalize Cortex for a gateway session whose agent left the cache."""
    from altas.cortex.lifecycle import finalize_detached_session
    from gateway.run import _profile_runtime_scope

    with _profile_runtime_scope(profile_home):
        return finalize_detached_session(
            profile_home,
            session_id,
            reason=reason,
        )
...
def _commit_detached_cortex_boundary(
    profile_home: Path,
    session_id: str,
    new_session_id: str,
    parent_session_id: str,
    ...
    """Atomically finalize and prepare a detached Cortex target lineage."""
    from altas.cortex.lifecycle import commit_detached_session_boundary
    from gateway.run import _profile_runtime_scope

    with _profile_runtime_scope(profile_home):
        return commit_detached_session_boundary(
            profile_home,
            session_id,
            new_session_id=new_session_id,
            parent_session_id=parent_session_id,
...
def _prepare_detached_cortex_branch(
    profile_home: Path,
    session_id: str,
    new_session_id: str,
    parent_session_id: str,
    ...
    """Create a detached child lineage without a semantic admission."""
    del reason, reset
    from altas.cortex.lifecycle import prepare_detached_session_branch
    from gateway.run import _profile_runtime_scope

    with _profile_runtime_scope(profile_home):
        return prepare_detached_session_branch(
            profile_home,
            session_id,
            new_session_id=new_session_id,
            parent_session_id=parent_session_id,
...
def _prepare_detached_cortex_resume(
    profile_home: Path,
    session_id: str,
    new_session_id: str,
    parent_session_id: str,
    ...
    """Preflight a detached resume without finalizing either conversation."""
    del parent_session_id, reason, reset
    from altas.cortex.lifecycle import prepare_detached_session_resume
    from gateway.run import _profile_runtime_scope

    with _profile_runtime_scope(profile_home):
        return prepare_detached_session_resume(
            profile_home,
            session_id,
            new_session_id=new_session_id,
        )
...
def _discard_detached_cortex_branch(
    profile_home: Path,
    session_id: str,
    new_session_id: str,
) -> bool:
    """Compensate one unpublished empty Cortex branch without distillation."""
    from altas.cortex.lifecycle import discard_detached_session_branch
    from gateway.run import _profile_runtime_scope

    with _profile_runtime_scope(profile_home):
        return discard_detached_session_branch(
            profile_home,
            session_id,
            new_session_id=new_session_id,
        )
...
def _reconcile_detached_cortex_rewind(
    profile_home: Path,
    session_id: str,
    source_row_ids: list[int | str],
) -> bool:
    """Reconcile Cortex after /undo when no cached provider can do it."""
    from altas.cortex.lifecycle import reconcile_detached_rewind
    from gateway.run import _profile_runtime_scope

    with _profile_runtime_scope(profile_home):
        return reconcile_detached_rewind(
            profile_home,
            session_id,
            source_row_ids,
        )
...
class GatewaySlashCommandsMixin:
    ...
    async def _commit_memory_boundary_before_session_switch(
        self,
        *,
        ...
        parent_session_id: str = "",
        reset: bool = False,
        require_cached_session_match: bool = False,
    ) -> bool:
        """Own one exact Cortex lifecycle transition.

        A cached provider is preferred because it holds the exact profile and
        transcript context. If soft eviction already removed it, the detached
        profile-scoped path updates the same brain idempotently. Branch and
        resume are topology-only and never finalize their source; explicit
        reset/new/end paths retain their terminal-boundary behavior. Semantic
        reset callers must first publish a durable predecessor token; topology
        callers use this as preflight and compensate if their routing CAS loses.
        ``False`` means no boundary was owned.
        """
        ...
        agent_memory_expected = bool(
            cached_agent is not None
            and getattr(cached_agent, "_memory_manager", None) is not None
        )
        if agent_memory_expected and require_cached_session_match:
            expected_session_id = str(getattr(session_entry, "session_id", "") or "")
            manager = getattr(cached_agent, "_memory_manager", None)
            cached_session_ids: set[str] = set()
            for candidate in (
                getattr(cached_agent, "session_id", None),
                getattr(manager, "_session_id", None),
            ):
                if isinstance(candidate, str) and candidate:
                    cached_session_ids.add(candidate)
            try:
                providers = getattr(manager, "providers", ())
                if isinstance(providers, (list, tuple)):
                    for provider in providers:
                        candidate = getattr(provider, "_session_id", None)
                        if isinstance(candidate, str) and candidate:
                            cached_session_ids.add(candidate)
            except Exception:
                pass

            # The cache is keyed by the stable routing key, not session id. A
            # rebuilt agent may therefore belong to the fresh session while a
            # persisted predecessor is still pending. Never finalize that
            # current provider on behalf of the predecessor; use the detached,
            # session-addressed path unless identity matches confidently.
            if cached_session_ids != {expected_session_id}:
                logger.debug(
                    "Skipping cached memory provider for %s boundary: expected "
                    "session %s, cached identities=%s",
                    reason,
                    expected_session_id,
                    sorted(cached_session_ids),
                )
                agent_memory_expected = False
        # Resume publication is a routing CAS followed by a fresh agent build.
        # Do not rebind the cached source provider before that CAS: detached
        # preflight below validates the target without reopening it, and the
        # newly built target provider performs the authoritative reopen.
        if reason == "resume":
            agent_memory_expected = False

        if agent_memory_expected:
            try:
                if new_session_id:
                    transition = (
                        _prepare_agent_memory_branch
                        if reason == "branch"
                        else _commit_agent_memory_boundary
                    )
                    transition_work = self._run_in_executor_with_context(
                        transition,
                        cached_agent,
                        new_session_id,
                        parent_session_id,
                    ...
                    # Executor work cannot be cancelled once its worker has
                    # started.  Keep lifecycle ownership until the provider
                    # returns definitively: timing out here could let a late
                    # semantic admission land after the caller reported that
                    # the route was unchanged (or let a late branch recreate
                    # a child after compensation).
                    await transition_work
                    return True
                await self._run_in_executor_with_context(
                    _finalize_agent_memory_boundary,
                    cached_agent,
                    reason,
                )
                return True
                ...
                logger.warning(
                    "Cached memory finalization failed during %s; trying the "
                    "detached Cortex boundary",
                    reason,
                    exc_info=True,
                )

        if session_entry is None or not getattr(session_entry, "session_id", None):
            return not agent_memory_expected
        try:
            origin = getattr(session_entry, "origin", None)
            boundary_source = origin if isinstance(origin, SessionSource) else source
            profile_home = self._resolve_profile_home_for_source(boundary_source)
            if new_session_id:
                if reason == "branch":
                    transition = _prepare_detached_cortex_branch
                elif reason == "resume":
                    transition = _prepare_detached_cortex_resume
                else:
                    transition = _commit_detached_cortex_boundary
                transition_work = self._run_in_executor_with_context(
                    transition,
                    profile_home,
                ...
                detached_owned = await transition_work
                return bool(detached_owned)
            detached_owned = await self._run_in_executor_with_context(
                _finalize_detached_cortex_boundary,
                profile_home,
                str(session_entry.session_id),
                reason,
            )
            ...
            return not cortex_boundary_required(profile_home)
        except Exception:
            logger.warning(
                "Detached Cortex finalization failed during %s; session switch "
                "was aborted",
                reason,
                exc_info=True,
            )
    ...
    async def _compensate_unpublished_memory_branch(
        self,
        *,
        source: SessionSource,
        session_key: str,
        ...
        """Restore the source binding and hide a child whose route CAS lost."""
        ...
        if cached_agent is not None:
            try:
                await self._run_in_executor_with_context(
                    _restore_agent_memory_branch_source,
                    cached_agent,
                    source_session_id,
                    child_session_id,
                )
                ...
                restored = False
                logger.error(
                    "Could not restore the source provider after branch routing lost",
                    exc_info=True,
                )
                # Remove the ambiguous runtime. The next source turn rebuilds
                # it from the unchanged route instead of writing through a
                # provider that may still be bound to the child.
        ...
        try:
            ...
            discarded = bool(
                await self._run_in_executor_with_context(
                    _discard_detached_cortex_branch,
                    profile_home,
                    source_session_id,
                    child_session_id,
                )
            ...
            logger.error(
                "Could not hide unpublished Cortex branch %s",
                child_session_id,
                exc_info=True,
            )
        return restored and discarded

    async def _finalize_previous_session_before_turn(
        self,
        *,
        source: SessionSource,
        session_entry: Any,
    ) -> bool:
        """Finalize a routed predecessor boundary before admitting a new turn.

        Auto-reset, explicit reset, and context-exhaustion reset publish a new
        routing entry before semantic work.  The fresh entry therefore carries
        ``previous_session_id`` as a durable retry token. Finalize a copy
        addressed to that prior identity, then compare-and-clear the token only
        after the Cortex boundary is owned successfully.
        """

        previous_session_id = getattr(session_entry, "previous_session_id", None)
        ...
        if not session_key or not current_session_id:
            logger.warning(
                "Pending auto-reset boundary %s has no current routing identity; "
                "new turn was refused.",
                previous_session_id,
            )
            return False

        previous_entry = dataclasses.replace(
            session_entry,
            session_id=previous_session_id,
            previous_session_id=None,
            previous_finalize_reason=None,
        )
        boundary_reason = (
            getattr(session_entry, "previous_finalize_reason", None)
            or "session_expired"
        )
        owned = await self._commit_memory_boundary_before_session_switch(
            source=source,
            session_key=session_key,
            session_entry=previous_entry,
            reason=boundary_reason,
            require_cached_session_match=True,
        )
        if not owned:
            logger.warning(
                "Routing memory boundary remains pending for previous session "
                "%s; new turn on %s was refused.",
                previous_session_id,
                current_session_id,
            )
            return False
        ...
        try:
            cleared = self.session_store.clear_previous_session_id(
                session_key,
            ...
            logger.warning(
                "Could not persist completion of auto-reset memory boundary %s; "
                "the idempotent boundary will retry on the next inbound turn.",
                previous_session_id,
                exc_info=True,
            )
            return False
        if not cleared:
            logger.warning(
                "Routing changed while finalizing previous session %s; "
                "new turn was refused so the durable retry token is preserved.",
                previous_session_id,
            )
            return False
        return True
    ...
    async def _handle_reset_command(
        ...
        if (
            ...
            and not await self._finalize_previous_session_before_turn(
                source=source,
                session_entry=old_entry,
            )
        ):
            return EphemeralReply(
                "Could not durably finalize Atlas memory; the session was left "
                "unchanged. Please retry /new."
            )
        # Snapshot the old runtime before publishing the replacement route.
        # Resource cleanup remains bounded, but semantic finalization below is
        # deliberately not: an executor worker cannot be cancelled safely.
        _old_agent = None
        ...
        # Publish the replacement first with an exact-current-session CAS.
        # The predecessor token blocks the fresh route from accepting a turn
        # until the exact old identity is finalized and compare-and-cleared.
        # This is the semantic authority: a failed/lost CAS cannot enqueue any
        # Cortex model work, and a crash after the CAS retains a durable retry.
        new_entry = None
        memory_boundary_acknowledged = old_entry is None
        if old_entry is not None:
            ...
            if new_entry is None:
                return EphemeralReply(
                    "The session route changed before /new could reserve its "
                    "replacement, so no Atlas memory work was started. Please retry."
                )

            try:
                memory_boundary_owned = (
                    await self._commit_memory_boundary_before_session_switch(
                        source=source,
                        session_key=session_key,
                        session_entry=old_entry,
                        reason="new_session",
                ...
                if memory_boundary_owned:
                    memory_boundary_acknowledged = bool(
                        self.session_store.clear_previous_session_id(
                            session_key,
                            current_session_id=new_entry.session_id,
                            previous_session_id=old_entry.session_id,
                        )
                    )
            except Exception:
                logger.warning(
                    "Atlas memory finalization remains pending after /new reserved "
        ...
        # daemons, background processes) before evicting from cache.  This is
        # nonsemantic cleanup, so it retains the bounded #35994 behavior.
        if _old_agent is not None:
            try:
                ...
                # Resource teardown is best-effort and may safely finish after
                # the route rotates; it cannot authorize Cortex work.
                logger.warning(
        ...
        if not memory_boundary_acknowledged:
            header += (
                "\n\nThe new session is reserved, but Atlas is still waiting to "
                "durably acknowledge the prior session's memory boundary. New "
                "turns will retry that exact boundary before they run."
            )
    ...
    async def _handle_undo_command(self, event: MessageEvent) -> str:
        ...
        session_key = self._session_key_for_source(source)
        manager_reconciled = False
        cached_manager_present = False
        cached_failure: Exception | None = None
        ...
        try:
            ...
            if manager is not None:
                ...
                manager_reconciled = True
        except Exception as e:
            # Detached reconciliation is idempotent and is the safe fallback
            # when a provider disappears or rejects its session-switch hook.
            logger.warning("undo: cached memory reconciliation failed: %s", e)
            cached_failure = e

        if not manager_reconciled:
            try:
                boundary_source = session_entry.origin or source
                profile_home = self._resolve_profile_home_for_source(boundary_source)
                detached_reconciled = await self._run_in_executor_with_context(
                    _reconcile_detached_cortex_rewind,
                    profile_home,
                    session_entry.session_id,
                    list(result.get("rewound_message_ids", [])),
                )
                # False is a clean "no Cortex state exists" result only when
                # there was no cached durable manager claiming otherwise.
                if cached_manager_present and not detached_reconciled:
                    reconciliation_failure = cached_failure or RuntimeError(
                        "durable rewind could not be reconciled"
                    )
                else:
                    manager_reconciled = True
            except FileNotFoundError as e:
                if cached_manager_present:
                    reconciliation_failure = cached_failure or e
                else:
                    manager_reconciled = True
            except Exception as e:
                logger.warning(
                    "Detached Cortex rewind reconciliation failed for session %s: %s",
                    session_entry.session_id,
                    e,
                )
                reconciliation_failure = e

        if reconciliation_failure is not None:
            row_ids = list(result.get("rewound_message_ids", []))
            compensated = self.session_store.restore_rewind(
                session_entry.session_id, row_ids
            )
            if not compensated:
                logger.critical(
                    "undo: SessionDB compensation failed for session %s",
                    session_entry.session_id,
                )
                return (
                    "Undo could not be completed safely. Restart Atlas before "
                    "continuing this session."
                )
            return "Undo was not applied; the session was left unchanged."

        # Transcript and durable memory now agree. Only now invalidate the
        # cached context and reset accounting for the shortened transcript.
        session_entry.last_prompt_tokens = 0
        self._evict_cached_agent(session_key)
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        ...
        if not await self._finalize_previous_session_before_turn(
            source=source,
            session_entry=current_entry,
        ):
            return (
                "Could not durably prepare Atlas memory; the current session "
                "was left unchanged. Please retry /resume."
            )
        if current_entry.session_id == target_id:
            return t("gateway.resume.already_on", name=name)

        if not await self._commit_memory_boundary_before_session_switch(
            source=source,
            session_key=session_key,
            session_entry=current_entry,
            reason="resume",
        ...
        new_entry = self.session_store.switch_session(
            session_key,
            target_id,
            expected_session_id=current_entry.session_id,
        )
        if not new_entry:
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        if not await self._finalize_previous_session_before_turn(
            ...
            return (
                "Could not durably prepare Atlas memory; the branch was not "
                "created. Please retry /branch."
            )
        ...
        if not await self._commit_memory_boundary_before_session_switch(
            ...
            compensated = await self._compensate_unpublished_memory_branch(
                source=source,
                session_key=session_key,
            ...
            if compensated:
                try:
                    await self._session_db.delete_session_if_empty(new_session_id)
                except Exception:
                    logger.error(
                        "Could not delete inactive branch after compensated "
                        "memory preparation failure",
                        exc_info=True,
                    )
            else:
                # The route still points at the source, but the cached runtime
                # may be ambiguously rebound. Force a clean rebuild on retry.
                self._evict_cached_agent(session_key)
                logger.error(
                    "Branch memory preparation failed and compensation was incomplete "
                    "for %s",
                    new_session_id,
                )
            return (
                "Could not durably prepare Atlas memory; the branch was not "
                "activated. Please retry /branch."
            )
        ...
        if not new_entry:
            ...
            if compensated:
                try:
                    ...
                    logger.error(
                        "Could not delete unpublished SessionDB branch %s",
                        new_session_id,
                        exc_info=True,
                    )
