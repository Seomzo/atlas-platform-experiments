...
def _cli_exit_memory_boundary_attempted(agent, session_id: str | None) -> bool:
    """Return whether interactive shutdown already owns this memory boundary.

    Interactive ``run()`` must commit (or deliberately reconcile) the Cortex
    boundary before it mutates SessionDB.  The process-wide cleanup hook runs
    afterward and historically finalized the same provider a second time.
    Keep the marker on the agent so the atexit/signal safety net can distinguish
    an owned interactive attempt from one-shot and partially initialized paths.
    """

    if agent is None:
        return False
    state = getattr(agent, "_cli_exit_memory_boundary", None)
    return bool(
        isinstance(state, dict)
        and state.get("session_id") == session_id
        and state.get("status") in {"attempted", "finalized", "reconciled"}
    )
...
def _run_cleanup(*, notify_session_finalize: bool = True):
    """Run resource cleanup exactly once without inventing a session end.

    Explicit interactive/one-shot exit paths own logical finalization before
    calling this function. Atexit, signal, broken-stdin, and interpreter
    teardown reach this function too, so it may checkpoint deterministically
    but must never admit Cortex semantic work on its own. The keyword remains
    for compatibility with older callers and is intentionally ignored.
    """
    del notify_session_finalize
    ...
    try:
        ...
        pass
    # Shut down memory providers as a resource operation. Explicit CLI close
    # already performed the true boundary; generic process cleanup only runs a
    # deterministic compatibility checkpoint and cannot authorize semantic
    # consolidation.
    cleanup_session_id = _active_agent_ref.session_id if _active_agent_ref else None
    interactive_boundary_attempted = _cli_exit_memory_boundary_attempted(
        _active_agent_ref,
        cleanup_session_id,
    )
    try:
        if (
            _active_agent_ref
            and not interactive_boundary_attempted
            ...
            if isinstance(_session_msgs, list):
                ...
                try:
                    ...
                    # Compatibility for lightweight test/plugin agent stubs
                    # that still expose the historical one-argument method.
...
def _finalize_single_query(cli) -> None:
    """Explicitly end one one-shot session, then release its resources."""
    try:
        close_session = getattr(cli, "_close_active_session_for_exit", None)
        if callable(close_session):
            close_session()
        else:
            # Compatibility for lightweight embedded/test CLI stubs. Real
            # HermesCLI instances always use the ordered durable close above.
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def __init__(
        ...
        # Only an explicit user-owned close (for example /exit, /quit, idle
        # Ctrl+C/Ctrl+Q, or EOF via Ctrl+D) may publish a logical session end.
        # Signals, broken stdin, relaunches, and unexpected UI teardown are
        # resource shutdowns: they leave the session active and resumable.
        self._logical_close_requested = False
        self._resource_shutdown_requested = False
    ...
    def _claim_active_session(
        ...
        try:
            ...
            message = (
                "Session ownership registry is unavailable; refusing to start "
                "an unprotected session."
            )
            if stderr:
    ...
    def _launch_session_boundary_memory_flush(
        ...
        *,
        ...
        """Deliver the old-session boundary to the local context engine.
        ...
        Memory-provider finalization is committed separately before this helper
        runs.  In particular, native Cortex only enqueues its cheap-model job
        at this logical boundary; no model call occurs inline here.

        Returns the frozen history for compatibility with older callers, or
        ``None`` when there is no agent/history/provider manager.
        """
        agent = getattr(self, "agent", None)
    ...
    def new_session(self, silent=False, title=None):
        ...
        _boundary_snapshot = copy.deepcopy(self.conversation_history or [])
        ...
        # Allocate the identity first, but do not expose it to the CLI, agent,
        # SessionDB, or hooks until every synchronous-durability provider has
        # captured and finalized the old logical session.  Cortex performs only
        # local durable work here (capture + immutable distillation enqueue);
        # the cheap model runs later on its own supervisor.
        ...
        if self.agent:
            ...
            if manager is not None:
                try:
                    ...
                    _cprint(
                        "  Could not durably finalize the current Atlas memory; "
                        "the session was left unchanged."
                    )
                    return

            # The context engine boundary is cheap and ordering-sensitive.  It
            # runs only after the durable provider boundary succeeds, but still
            # before reset_session_state() mutates its old-session state.
            ...
            if self._session_db:
                ...
                if title and self._session_db:
                    ...
                    if sanitized:
                        ...
                        title = None
            # Provider finalization and rebinding already completed (or were
            # queued for non-durable legacy providers) before session mutation.
            # The plugin reset hook therefore observes one committed boundary.
    ...
    def undo_last(self, n: int = 1, prefill: bool = True):
        ...
        rewound_rows = 0
        rewound_message_ids = []
        canonical_rewind_committed = False
        if self._session_db is not None and self.session_id:
            try:
                ...
                if recents:
                    ...
                    canonical_rewind_committed = True
                ...
                logger.warning("undo: canonical rewind rejected: %s", e)
                print("(._.) Undo was not applied; the session was left unchanged.")
                return
            except Exception as e:
                logger.warning("undo: canonical rewind failed: %s", e, exc_info=True)
                print("(._.) Undo was not applied; the session was left unchanged.")
                return

        # Durable Cortex reconciliation is part of the same user-visible
        # operation. If it fails after SessionDB's soft-delete, compensate the
        # exact row-ID receipt before touching in-memory history.
        try:
            ...
            if _mm is not None and self.session_id:
                _mm.on_session_switch(
                    ...
                    reset=False,
                    rewound=True,
                    rewound_row_ids=rewound_message_ids,
                )
        except Exception as e:
            compensated = not canonical_rewind_committed
            if canonical_rewind_committed and self._session_db is not None:
                try:
                    compensated = self._session_db.restore_rewound_ids(
                        self.session_id, rewound_message_ids
                    ) == len(set(rewound_message_ids))
                except Exception:
                    compensated = False
                    logger.error(
                        "undo: SessionDB compensation failed",
                        exc_info=True,
                    )
            logger.error("undo: durable memory reconciliation failed: %s", e)
            if compensated:
                print("(._.) Undo was not applied; the session was left unchanged.")
            else:
                print(
                    "(._.) Undo could not be completed safely. Restart Atlas before "
                    "continuing this session."
                )
            return

        # Both canonical transcript and required memory state now agree.
    ...
    def process_command(self, command: str) -> bool:
        ...
        if canonical in {"quit", "exit"}:
            ...
            if _args in {"--delete", "-d"}:
                ...
                return True
            self._logical_close_requested = True
            return False
        elif canonical == "help":
            self.show_help()
    ...
    def _billing_poll_charge(self, state, charge_id, amount):
        ...
        print(
            "  🟡 Still processing after 5 minutes — this is a timeout, not a "
    ...
    def _wait_for_active_turn_before_exit(self, timeout: float = 10.0) -> bool:
        """Wait for an interrupted turn to publish its durability boundary.

        A forced Ctrl+C can tear prompt_toolkit down while ``chat()`` is still
        unwinding on the process thread.  Finalizing Cortex during that window
        races the turn's own terminal capture and can admit an incomplete
        evidence epoch.  ``_agent_running`` is cleared only after
        ``run_conversation`` returns (including its synchronous Cortex capture),
        so it is the classic CLI's durability barrier.
        """
    ...
    def _exit_memory_messages(self) -> list:
        """Return the authoritative in-process transcript for an exit boundary."""

        agent = getattr(self, "agent", None)
        messages = getattr(agent, "_session_messages", None) if agent else None
        if isinstance(messages, list):
            return messages
        history = getattr(self, "conversation_history", None)
        return history if isinstance(history, list) else []

    def _exit_session_message_row_ids(self, session_id: str) -> list[int]:
        """Collect every stable SessionDB row id, including compacted/rewound rows."""

        row_ids: set[int] = set()
        session_db = getattr(self, "_session_db", None)
        get_messages = getattr(session_db, "get_messages", None)
        if callable(get_messages):
            rows = get_messages(session_id, include_inactive=True)
            for row in rows or []:
                row_id = row.get("id") if isinstance(row, dict) else None
                if isinstance(row_id, int):
                    row_ids.add(row_id)
        for message in self._exit_memory_messages():
    ...
    @staticmethod
    def _record_cli_exit_memory_boundary(agent, session_id: str, status: str) -> None:
        try:
            agent._cli_exit_memory_boundary = {
                "session_id": session_id,
                "status": status,
            }
        except Exception:
    ...
    def _finalize_memory_before_cli_close(self, session_id: str) -> None:
        """Commit one true logical boundary before publishing SessionDB end."""

        agent = getattr(self, "agent", None)
        if agent is None:
            return
        existing = getattr(agent, "_cli_exit_memory_boundary", None)
        if isinstance(existing, dict) and existing.get("session_id") == session_id:
            if existing.get("status") == "finalized":
                return
            raise RuntimeError("the CLI memory boundary was already attempted")

        self._record_cli_exit_memory_boundary(agent, session_id, "attempted")
        manager = getattr(agent, "_memory_manager", None)
        flush_pending = getattr(manager, "flush_pending", None)
        if callable(flush_pending):
            # Cortex capture is synchronous; this bounded barrier preserves the
            # existing best-effort provider ordering before shutdown.
            try:
                flush_pending(timeout=10)
            except Exception:
                logger.debug(
                    "CLI exit could not drain best-effort memory work before "
        ...
        if callable(shutdown):
            shutdown(
                self._exit_memory_messages(),
                finalize=True,
                reason="cli_close",
            )
        elif manager is not None:
            raise RuntimeError("the active memory manager cannot be finalized")

        self._record_cli_exit_memory_boundary(agent, session_id, "finalized")
        _notify_session_finalize(
            session_id=session_id,
    ...
    def _reconcile_memory_before_cli_delete(self, session_id: str) -> None:
        """Remove the transcript's Cortex evidence without admitting distillation.

        ``/exit --delete`` is a privacy operation, not an ordinary logical
        finalization.  Reuse the same row-provenance rewind reconciliation as
        ``/undo`` so already-captured evidence and its projections are removed,
        skip every semantic-finalize hook, then tear providers down.  If the
        exact persisted rows cannot be identified or reconciled, the caller
        must leave the ordinary transcript untouched.
        """
        ...
        self._record_cli_exit_memory_boundary(agent, session_id, "attempted")
        messages = self._exit_memory_messages()
        manager = getattr(agent, "_memory_manager", None)
        try:
            unpersisted = [
                message
                ...
                and not isinstance(message.get("_db_row_id"), int)
                and not any(
                    message.get(flag)
                    for flag in (
            ...
            if unpersisted:
                raise RuntimeError(
                    "the live transcript has messages without persisted row provenance"
                )
            row_ids = self._exit_session_message_row_ids(session_id)
            if messages and not row_ids:
                raise RuntimeError(
                    "the persisted transcript row identities are unavailable"
                )

            if manager is not None:
                flush_pending = getattr(manager, "flush_pending", None)
                if callable(flush_pending) and not flush_pending(timeout=10):
                    raise RuntimeError("pending memory capture did not drain")

                raw_providers = getattr(manager, "providers", [])
                try:
                ...
                unsupported = [
                    ...
                    if getattr(provider, "name", "") not in {"builtin", "cortex"}
                ]
                if (messages or row_ids) and unsupported:
                    raise RuntimeError(
                        "the configured memory provider has no transcript deletion primitive"
                    )

            if row_ids:
                get_provider = getattr(manager, "get_provider", None)
                cortex_provider = (
                    get_provider("cortex") if callable(get_provider) else None
                )
                if getattr(cortex_provider, "name", None) == "cortex":
                    reconcile = getattr(manager, "on_session_switch", None)
                    if not callable(reconcile):
                        raise RuntimeError("Cortex reconciliation is unavailable")
                    reconcile(
                        session_id,
                        parent_session_id="",
                        reset=False,
                        rewound=True,
                        rewound_row_ids=row_ids,
                    )
                else:
                    # A provider may already have left the active manager (for
                    # example after a degraded restart).  The profile-scoped
                    # detached primitive safely returns False when no Cortex
                    # session exists, and raises on an actual reconciliation
                    # failure.
                    from altas.cortex.lifecycle import reconcile_detached_rewind
                    from altas.cortex.config import CortexConfig
                    from hermes_constants import get_hermes_home

                    profile_home = get_hermes_home()
                    reconciled = reconcile_detached_rewind(
                        profile_home,
                        session_id,
                        row_ids,
                    )
                    if (
                        getattr(agent, "_cortex_memory_active", False) is True
                        or CortexConfig.load(profile_home).database_path.exists()
                    ) and not reconciled:
                        raise RuntimeError(
                            "the profile has Cortex state that could not be reconciled"
                        )
        finally:
        ...
        self._record_cli_exit_memory_boundary(agent, session_id, "reconciled")

    def _close_active_session_for_exit(self) -> bool:
        """Own memory ordering, then end or delete the interactive transcript.

        Returns ``False`` when the durable/privacy boundary failed.  In that
        case no SessionDB end/delete is published, leaving the transcript open
        for explicit recovery rather than silently losing the logical boundary.
        """
        ...
        if not session_id:
            return True
        delete_requested = bool(getattr(self, "_delete_session_on_exit", False))
        try:
            turn_durable = self._wait_for_active_turn_before_exit()
        ...
        if not turn_durable:
            ...
            if delete_requested:
                _cprint(
                    "  The active turn did not finish its durability barrier; "
                    "the session transcript was left intact."
                )
            else:
                _cprint(
                    "  The active turn did not finish its durability barrier; "
                    "the session transcript was left open for recovery."
                )
            return False

        # The agent thread can own messages that never reached its normal
        # per-turn persistence path (SIGHUP/window close). Persist only after it
        # is quiescent, and still before either Cortex or SessionDB is ended.
        self._persist_active_session_before_close()
        try:
            if not delete_requested:
                self._finalize_memory_before_cli_close(session_id)
        except (Exception, KeyboardInterrupt) as exc:
            logger.error(
                "CLI exit memory boundary failed for session %s: %s",
                session_id,
                exc,
                exc_info=True,
            )
            if delete_requested:
                _cprint(
                    "  Could not safely reconcile Atlas memory for deletion; "
                    "the session transcript was left intact."
                )
            else:
                _cprint(
                    "  Could not durably finalize Atlas memory; the session "
                    "transcript was left open for recovery."
                )
            return False
        ...
        if session_db is None:
            if delete_requested:
                _cprint(
                    "  Session deletion was not performed because the transcript "
        ...
        if delete_requested:
            try:
                ...
                profile_home = _ghh()
                own_lease = getattr(self, "_active_session_lease", None)
                ignore_lease_ids = (
                    [str(own_lease.lease_id)]
                    if own_lease is not None
                    and getattr(own_lease, "lease_id", None)
                    else []
                )
                with claim_session_deletion(
                    [session_id],
                    active_aliases=session_activity_aliases(
                        session_db, [session_id]
                    ),
                    hermes_home=profile_home,
                    ignore_lease_ids=ignore_lease_ids,
                ) as deletion:
                    self._reconcile_memory_before_cli_delete(session_id)
                    deletion.seal()
                    sessions_dir = profile_home / "sessions"
                    if session_db.delete_session(
                        session_id, sessions_dir=sessions_dir
                    ):
                        _cprint(
                            f"  {_DIM}✓ Session {_escape(session_id)} deleted{_RST}"
                        )
                    else:
                        _cprint(
                            f"  {_DIM}✗ Session {_escape(session_id)} not found for deletion{_RST}"
                        )
            except (Exception, KeyboardInterrupt) as exc:
                logger.error("Could not delete session on exit: %s", exc, exc_info=True)
                _cprint(
                    "  Could not safely reconcile Atlas memory for deletion; "
                    "the session transcript was left intact or tombstoned for retry."
                )
                return False
            return True
        ...
        try:
            ...
            logger.debug("Could not close session in DB: %s", exc)
            return False
        try:
            self._discard_session_if_empty(session_id)
        except (Exception, KeyboardInterrupt) as exc:
            logger.debug("Could not prune empty session: %s", exc)
        return True

    def _apply_interactive_exit_boundary(self) -> tuple[bool, bool]:
        """Close only on explicit intent; otherwise preserve for resume.

        Returns ``(logical_close, boundary_ok)``. Process signals override a
        previously staged UI intent because a signal-driven unwind cannot
        prove that the normal close command reached its durable boundary.
        """

        logical_close = bool(
            getattr(self, "_logical_close_requested", False)
            and not getattr(self, "_resource_shutdown_requested", False)
        )
        if logical_close:
            return True, self._close_active_session_for_exit()

        try:
            turn_quiescent = self._wait_for_active_turn_before_exit()
        except (Exception, KeyboardInterrupt):
            turn_quiescent = False
        if turn_quiescent:
            self._persist_active_session_before_close()
        return False, False
    ...
    def run(self):
        ...
        def _signal_handler(signum, frame):
            ...
            # A process signal is operational teardown, not proof that the
            # customer intentionally completed this logical conversation.
            # Preserve its deterministic transcript/checkpoint for resume and
            # prohibit the Cortex semantic model from being admitted here.
            self._resource_shutdown_requested = True
            try:
        ...
        try:
            ...
            try:
                _shutdown_detail = (
                    "finalizing session"
                    if _logical_close
                    else "preserving session for resume"
                )
                print(
                    f"{_DIM}Shutting down… ({_shutdown_detail}){_RST}",
                    flush=True,
                )
            except Exception:
                pass
            ...
            set_approval_callback(None)
            set_secret_capture_callback(None)
            # Only a deliberate user close owns the durable semantic boundary.
            # Operational teardown persists what is already durable and lets
            # generic cleanup checkpoint providers with ``finalize=False``;
            # it never ends SessionDB or admits Cortex distillation.
            _logical_close, _session_boundary_ok = (
                self._apply_interactive_exit_boundary()
            )
            ...
            if (
                _session_boundary_ok
                and not getattr(self, "_delete_session_on_exit", False)
                and self.agent
                and getattr(self, "_agent_running", False)
            ):
                try:
                    from hermes_cli.plugins import invoke_hook as _invoke_hook
