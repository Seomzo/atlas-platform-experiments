...
from agent.memory_manager import MemoryDurabilityError, sanitize_context
...
_EPHEMERAL_SCAFFOLDING_FLAGS = (
    ...
    # Codex intent-ack recovery inserts an internal user continuation after an
    # acknowledgement-only response. It is API scaffolding, not a customer
    # message, and must not survive into durable transcripts.
...
# Stable SQLite message identity used by first-party durability consumers such
# as Atlas Cortex.  Like the persisted marker, this is process-internal and is
# stripped by every wire adapter before a model request is sent.
_DB_ROW_ID_MARKER = "_db_row_id"
...
class AIAgent:
    ...
    def _spawn_background_review(
        ...
        if cortex_owns_semantic_review(self):
            # Cortex performs semantic memory work through the dedicated cheap
            # route only after the logical session ends. Do not let the legacy
            # background fork spend the conversational frontier model or write
            # MEMORY.md/skills mid-session under an Atlas Cortex profile.
            return
    ...
    def _flush_messages_to_session_db(
        ...
        try:
            ...
            for _msg_idx, msg in enumerate(messages):
                ...
                msg[_DB_PERSISTED_MARKER] = True
                msg[_DB_ROW_ID_MARKER] = _db_row_id
    ...
    def shutdown_memory_provider(
        ...
        *,
        ...
        """Shut down memory/context resources, optionally finalizing the conversation.

        ``on_session_end`` remains a compatibility checkpoint and is safe for
        process shutdown/cache cleanup. Semantic finalization (and therefore a
        Cortex dream job) occurs only when ``finalize=True`` at a genuine
        logical boundary such as owned CLI exit, reset, or idle expiry.
        """
        durability_error: MemoryDurabilityError | None = None
        if self._memory_manager:
            if finalize:
                try:
                    self._memory_manager.on_session_finalize(
                        messages or [], reason=reason
                    )
                except MemoryDurabilityError as e:
                    durability_error = e
                    logger.error(
                        "Durable memory finalization failed during shutdown: %s",
                        e,
                        exc_info=True,
                    )
                except Exception as e:
                    logger.warning(
                        "Memory provider finalization failed during shutdown: %s",
                        e,
                        exc_info=True,
                    )
            try:
                # A durable provider's true finalization already captures the
                # exact terminal evidence epoch. Never run its legacy
                # checkpoint afterward: on failure that would invalidate the
                # recovery marker, and on success it would be a post-boundary
                # write. Best-effort providers may still flush for teardown.
                self._memory_manager.on_session_end(
                    messages or [], include_durable=not finalize
                )
            except Exception as e:
                logger.warning(
        ...
        if durability_error is not None:
            # Cleanup still ran, but callers must not report a durable logical
            # boundary as complete. Cortex has already persisted its retry
            # marker before the fallible final capture path.
    ...
    def commit_memory_session(
        ...
        *,
        ...
        ``checkpoint_memory=False`` lets a caller close context-engine state
        after it has already attempted the manager's true logical finalization.
        This avoids a second durable write after the recorded boundary epoch.
        """
        if self._memory_manager and checkpoint_memory:
            try:
    ...
    def _sync_external_memory_for_turn(
        ...
        *,
        ...
        Interrupted turns preserve the user's input and completed tool
        evidence with an explicit ``interrupted`` status, but never treat a
        partial assistant response as durable customer truth.  Next-turn
        prefetch remains disabled for interrupted work.

        Normal completed turns still sync as before. Best-effort providers may
        fail without blocking a response; a provider that declares durable
        capture instead raises ``MemoryDurabilityError`` and the completed turn
        must not be acknowledged as durably persisted.
        """
        if not (self._memory_manager and original_user_message):
            return
        ...
        response_text = (
            ""
            if interrupted
            else _summarize_user_message_for_log(final_response, sep="\n")
        )
        if not user_text or (not interrupted and not response_text):
            return
        try:
            ...
            for message in messages or []:
                if not isinstance(message, dict):
                    continue
                row_id = message.get(_DB_ROW_ID_MARKER)
                if isinstance(row_id, int):
                    source_row_ids.append(row_id)
                timestamp = message.get("timestamp")
                if timestamp is not None:
                    source_timestamps.append(str(timestamp))
            sync_kwargs["turn_metadata"] = {
                "session_id": self.session_id or "",
    ...
    def close(self) -> None:
        ...
        # temporary helper or Cortex owns the logical lifecycle. ``close()`` is
        # resource teardown, not proof of a true conversation boundary. Cortex
        # callers must explicitly commit durable final evidence first and only
        # then publish SessionDB end state; letting this generic primitive do it
        # would bypass that ordering whenever a primary caller forgot the
        # lifecycle hook. Non-Cortex agents retain the historical behavior.
        try:
            from agent.native_memory_providers import cortex_owns_semantic_review

            if (
                getattr(self, "_end_session_on_close", True)
                and not cortex_owns_semantic_review(self)
            ):
