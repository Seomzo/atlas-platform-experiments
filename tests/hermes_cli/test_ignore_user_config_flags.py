...
class TestIgnoreUserConfigEnvGate:
    ...
    # Unique sentinel: the repo root may carry a developer-local (git-ignored)
    # cli-config.yaml whose real model.default could collide with a realistic
    # model name, making the != assertions below vacuous/false. Never use a
    # real model id here.
    _SENTINEL_MODEL = "test-vendor/ignore-user-config-sentinel"

    def test_user_config_loaded_when_flag_unset(self, tmp_path, monkeypatch):
        self._write_user_config(tmp_path, self._SENTINEL_MODEL)
        load_cli_config = self._reload_cli(monkeypatch, tmp_path)

        cfg = load_cli_config()

        # User config value wins
        assert cfg["model"]["default"] == self._SENTINEL_MODEL
    ...
    def test_user_config_skipped_when_flag_set(self, tmp_path, monkeypatch):
        ...
        # user's value
        assert cfg["model"].get("default", "") != self._SENTINEL_MODEL
