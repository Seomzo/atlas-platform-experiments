import { AGENT_NAME, COMMAND_NAME } from '../brand.js'
...
export const buildSetupRequiredSections = (): PanelSection[] => [
  {
    text: `${AGENT_NAME} needs a model provider before the TUI can start a session.`
  },
  {
    rows: [
      ['/model', 'configure provider + model in-place'],
      ['/setup', 'run full first-time setup wizard in-place'],
      ['Ctrl+C', `exit and run \`${COMMAND_NAME} setup\` manually`]
    ],
    title: 'Actions'
  }
]
