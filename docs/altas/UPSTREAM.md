...
Hermes embeds its name in Python distributions and imports, command names,
environment variables, data paths, Docker layouts, installers, Electron bundle
IDs, protocol handlers, updater behavior, plugins, tests, and documentation.
A blanket rename would create high regression risk and permanent merge debt.

The prototype therefore rebrands the public product while retaining internal
compatibility names behind an Altas adapter. References to Hermes are permitted
only in:
...
prototype quick start use Altas. Legacy upstream commands remain available to
...
The Altas application has its own `0.1.0` version and public `altas` entry
point. The legacy commands are compatibility surfaces for engine maintenance;
they are not the intended dealership install or onboarding flow. A later
packaging milestone will place the pinned engine behind a dedicated Altas
distribution and signed desktop installer.
...
- Small and generic
- Covered by a focused Altas regression test
- Recorded below
...
| `altas` console entry point in `pyproject.toml` | Public product launcher | Package split moves engine under a dedicated submodule |
| Altas provider plugin | Route engine inference through Altas gateway | Retained as supported integration |

## Update process

1. Fetch a reviewed upstream release; do not float on `main` in production.
2. Record the candidate commit and release notes.
3. Run pristine upstream tests before applying Altas patches.
4. Run Altas control-plane, adapter, policy, worker, and UI tests.
...
The upstream `hermes update` flow is not a production Altas update mechanism.
Customer deployments will eventually use a signed Altas release feed.
